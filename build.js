...
function safeUrl(u) {
  // Só pra não poluir log: mostra domínio e path base
  try {
    const url = new URL(u);
...
async function build() {
  ...
  console.log("[1/3] Bundling (esbuild)...");
  const t1 = Date.now();
  ...
  const result = await esbuild.build({
    ...
    write: false,
    legalComments: "none",
    // Opcional: útil pra debug de bundle:
    // metafile: true,
    define: {
  ...
  console.log("[2/3] Minifying (terser)...");
  const t2 = Date.now();
  ...
  console.log("[3/3] Writing loader.js...");
  const t3 = Date.now();

  await writeFile("loader.js", out);
  console.log(`     done in ${fmtMs(Date.now() - t3)}`);

  console.log("");
  console.log(`✅ Build complete: loader.js (${fmtBytes(after)}) in ${fmtMs(Date.now() - t0)}`);
}
